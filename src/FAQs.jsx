...
function FaqCard({ item, isOpen, onToggle, lang, t }) {
  const answer = item.a !== null ? item.a : (
    <span className="faq__contact-links">
      <LocalizedText lang={lang} type="body">{t.faqs.contactText}</LocalizedText>{' '}
      <a href="tel:+251911979899" className="faq__link"><Phone size={13}/> +251 911 979 899</a>{' · '}
      <a href="tel:+251982323334" className="faq__link"><Phone size={13}/> +251 982 323 334</a>{' · '}
  ...
  return (
    <motion.div
      ...
      layout
    >
      <button className="faq__trigger" onClick={onToggle} aria-expanded={isOpen}>
        <span className="faq__question">{item.q}</span>
        <motion.span className="faq__icon" animate={{ rotate: isOpen ? 90 : 0 }}>
...
<div className='faq-container'>
  ...
  <div className="faq-sidebar">
    ...
    {['about', 'orders', 'storage', 'production'].map((catKey) => (
      <div key={catKey} className="category-group">
        ...
        <button 
          ...
          onClick={() => {
            if(activeCategory === catKey && isListVisible) setIsListVisible(false);
        ...
        <div className={`mobile-nested-list ${activeCategory === catKey && isListVisible ? 'show' : ''}`}>
    ...
    <div className="sidebar-footer">
      ...
      <p>{t.faqs.card.q6}</p>
      <a href='#contact' className="sidebar-btn active small-btn">CONTACT US</a>
    </div>
  </div>

  {/* --- DESKTOP ONLY: Questions appear on the right (> 1000px) --- */}
  <div className={`questions-list-desktop ${isListVisible ? 'visible' : ''}`}>
    <AnimatePresence mode="wait">
      <motion.div 
        ...
        style={{width: '800px', display: 'flex', flexDirection: 'column', gap: '18px' }}
      >
        ...
        {/* ── Mobile Layout ── */}
        <div className="faqs__mobile" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
          <AnimatePresence mode="wait">
            <motion.div
              key={mobIdx}
            ...
            >
              {FAQ_GROUPS[mobIdx].map((item) => (
                <FaqCard
                  ...
                  onToggle={() => setMobOpen(mobOpen === item.id ? null : item.id)}
                />
              ))}
            </motion.div>
          </AnimatePresence>
