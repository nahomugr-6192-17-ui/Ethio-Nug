    ...
    <div className="cform__stars">
      {[1, 2, 3, 4, 5].map((n) => (
        ...
        >
          <Star
            size={22}
            fill={n <= (hov || value) ? "currentColor" : "none"}
            strokeWidth={1.8}
          />
        </button>
      ))}
...
export default function Contact({ lang }) {
  ...
  const submit = async (e) => {
    ...
    // 1. Check required text fields first
    if (!form.name.trim() || !form.message.trim()) {
      toast.error(
        lang === "am"
          ? "እባክዎ የተፈለጉትን መስኮች ይሙሉ"
          : "Please fill in the required fields.",
      );
      return;
    }

    // 2. Then check the rating
    if (rating === 0) {
      toast.error(t.contact.form.ratingError);
      return;
    }
